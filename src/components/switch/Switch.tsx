...
const Switch: FC<SwitchProps> = ({custom, disabled, initialChecked, label, labelPosition = 'left', onClick, isSimple=false}) => {
  ...
  const classState = `toggle-${value ? 'on' : 'off'} ${custom}`;
  const labelPlacement = `label-${labelPosition}`;

  const computedClasses = clsx({'disabled': disabled});
